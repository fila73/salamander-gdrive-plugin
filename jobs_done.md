...
| **Find dialog – opravy (v0.4)** | • Oprava funkce Focus pro soubory ve *Sdíleno se mnou* a složky s českou diakritikou: přímé cachování ID→cesta při dohledávání výsledků eliminuje chybný fallback na `\My Drive`. • View (F3) nyní otevírá soubor v interním prohlížeči Salamandera (`ViewFileInPluginViewer`), nikoliv v systémové aplikaci. • `Enter` v libovolném editboxu/comboboxu spouští vyhledávání. • `Esc` spolehlivě zavírá dialog z jakéhokoliv ovládacího prvku. • Dialog je nemodální bez nucené horní vrstvy (`WS_EX_TOPMOST`). | `src/dialog_find.cpp`, `src/gdrive_fs_nav.cpp`, `src/gdrive_fs.h` |
